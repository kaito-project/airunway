...
func TestEngineCapability_RequiresInstallation(t *testing.T) {
	trueVal := true
	falseVal := false
	...
	tests := []struct {
		...
		{
			name: "nil RequiresCRD defaults to true",
			ec:   &EngineCapability{Name: EngineTypeVLLM},
			want: true,
		},
		{
			name: "explicit true",
			ec:   &EngineCapability{Name: EngineTypeVLLM, RequiresCRD: &trueVal},
			want: true,
		},
		{
			name: "explicit false opts out",
			ec:   &EngineCapability{Name: EngineTypeVLLM, RequiresCRD: &falseVal},
			want: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.ec.RequiresInstallation(); got != tt.want {
				t.Errorf("RequiresInstallation() = %v, want %v", got, tt.want)
			}
		})
	}
}
